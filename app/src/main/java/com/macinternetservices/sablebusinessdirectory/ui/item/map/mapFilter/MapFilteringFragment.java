package com.macinternetservices.sablebusinessdirectory.ui.item.map.mapFilter;
...
import com.macinternetservices.sablebusinessdirectory.databinding.FragmentMapFilteringBinding;
        ...
        if (item.getItemId() == R.id.apply) {
            ...
            if (getActivity() != null) {
                ...
                if (currentIndex != seekBarValues.length - 1) {
                    //itemParameterHolder.miles = String.valueOf(Utils.roundDouble(Float.parseFloat(seekBarValues[currentIndex]) * 0.621371, 3));
                    itemParameterHolder.miles = getMiles(Float.parseFloat(seekBarValues[currentIndex]));
                    preferences.edit().putString(Constants.RADIUS_KEY,seekBarValues[currentIndex]).commit();
                } else {
                    preferences.edit().putString(Constants.RADIUS_KEY,seekBarValues[currentIndex]).commit();
                    resetTheValues();
                }
                ...
                navigationController.navigateBackFromSearchFilterToAllListing(this.getActivity(), itemParameterHolder);
            }


        } else if (item.getItemId() == R.id.reset) {
            preferences.edit().putString(Constants.RADIUS_KEY,Constants.CONST_RADIUS).apply();
            resetTheValues();
    ...
    private void initializeMap(Bundle savedInstanceState) {
        ...
        if (getActivity() != null) {
            ...
            if(itemParameterHolder.lat.equals("") && itemParameterHolder.lng.equals("") ) {
                ...
                itemParameterHolder.miles = getMiles(100.0F);
            }
        }
...
//                        PERMISSIONS_REQUEST_ACCESS_FINE_LOCATION);
    ...
    private void updateUI(ItemParameterHolder itemParameterHolder) {
        ...
        if (!itemParameterHolder.lat.isEmpty() && !itemParameterHolder.lng.isEmpty() && !itemParameterHolder.miles.isEmpty()) {
            ...
            if (map != null) {
                ...
                currentIndex = findTheIndexOfTheValue(itemParameterHolder.miles);
