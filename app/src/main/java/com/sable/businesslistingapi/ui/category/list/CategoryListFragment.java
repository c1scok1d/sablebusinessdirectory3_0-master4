package com.sable.businesslistingapi.ui.category.list;
...
import com.sable.businesslistingapi.databinding.FragmentCategoryListBinding;
import com.sable.businesslistingapi.ui.category.adapter.CategoryAdapter;
...
public class CategoryListFragment extends PSFragment implements DataBoundListAdapter.DiffUtilDispatchedInterface {
                        ...
                        if (!binding.get().getLoadingMore() && !itemCategoryViewModel.forceEndLoading) {
                            ...
                            if (connectivity.isConnected()) {
                                ...
                                itemCategoryViewModel.setNextPageLoadingStateObj(String.valueOf(Config.LIST_CATEGORY_COUNT),
                                        String.valueOf(itemCategoryViewModel.offset), selectedCityId);//itemCategoryViewModel.categoryParameterHolder.cityId);
                            }
                        }
                    }
                }
    ...
    protected void initAdapters() {
        CategoryAdapter nvAdapter = new CategoryAdapter(dataBindingComponent,
                category -> {
//                    itemCategoryViewModel.productParameterHolder.catId = category.id;
//                    navigationController.navigateToHomeFilteringActivity(getActivity(), itemCategoryViewModel.productParameterHolder, category.name);
//
//                    if (connectivity.isConnected()) {
//                        touchCountViewModel.setTouchCountPostDataObj(loginUserId, category.id, Constants.FILTERING_TYPE_NAME_CAT, selectedCityId);
//                    }
                    ...
                    if (Config.SHOW_SUBCATEGORY) {
                        ...
                        navigationController.navigateToHomeFilteringActivity(CategoryListFragment.this.getActivity(), itemParameterHolder, category.name, selectedCityLat, selectedCityLng, selectedCityName);
                    }
